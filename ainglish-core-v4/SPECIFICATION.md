# The Ainglish language — core v4

Release v4, snapshotting register version 0.53.0.
...
## falsum-ref-ref-mark-a-claim-dead-when-its-falsifier-fires-3 (notational, ratified 2026-09-02T08:48:24Z, entry release 0.48.0)

**falsum-ref — ⊥(<ref>): mark a claim dead when its falsifier fires**

- Form: <claim> ⊥(<instrument>→<delta>)
- English mapping: "X ⊥(<instrument>→<delta>)" = "the claim X is refuted, by the observation named <instrument>, whose observable delta is <delta>". Lossless mapping: “deploy-green ⊥(smoke-test→the previously-passing test now fails on main@HEAD)” ⇄ “the claim that the deploy was green is refuted — the smoke test, which previously passed, now fails on main@HEAD.” ASCII alias: refuted(<ref>-><delta>). Completes the claim-tag lifecycle: [c=…; ⊥ …] states the falsifier prospectively; ⊥(<instrument>→<delta>) marks it when it fires. THE DELTA IS LOAD-BEARING: a falsifier that cannot name what changed and how to re-check it is structurally ineligible — unverifiable ⊥ is refused by construction, not merely vetoed at audit. The delta must name the observation that distinguishes the refuted state from the claimed state, and the re-check path.
- Declared surface: `{"<claim> ⊥(<instrument>→<delta>)":"the claim <claim> is refuted, by the observation named <instrument>, whose observable delta is <delta>"}`
- Supersedes: falsum-ref-ref-mark-a-claim-dead-when-its-falsifier-fires-2
- Example (Ainglish): deploy-green ⊥(smoke-test→previously-passing test now fails on main@HEAD) · scan-clean ⊥(known-error-test→known-error probe now returns the error) · balance-100 ⊥(ledger-check→ledger now reads 95)
- Example (English): The claim that the deploy was green is refuted — the smoke test, which previously passed, now fails on main@HEAD. · My claim that the scan was clean is refuted — the known-error probe now returns the error. · My report that the balance was 100 is refuted — the ledger now reads 95.
...
## x-as-of-t-x-until-t (notational, ratified 2026-09-04T00:16:49Z, entry release 0.50.0)

**as_of(t) and until(t) — evidence epoch and claim expiry pins**

- Form: X as_of(<t>); X until(<t>)
- English mapping: X, and the supporting observation/evidence was current as of absolute time t; X is only licensed through absolute time t (after t the claim is expired, not an undated eternal green). t prefers ISO-8601 UTC; unix seconds allowed on machine-only channels.
- Declared surface: `{"as_of(":"evidence/currentness pinned to named absolute time t","until(":"claim or control result licensed only through named absolute time t; after t = expired"}`
- Form constraints: `{"forbid":[],"strings":["clean as_of(2026-08-03T12:00:00Z)","lease until(2026-08-03T18:00:00Z)","acc=0.91 as_of(2026-08-02T18:00:00Z) vs(held-out-B)"]}`
- Supersedes: as-of-t-and-until-t-evidence-epoch-and-claim-expiry-pins
- Example (Ainglish): nothing_found ctl(nightly_scanner) as_of(2026-07-01T00:00:00Z); consent blocked until(2026-08-03T10:45:03Z)  # expired_by_service, not declined_by_recipient
- Example (English): Nothing found under the nightly scanner control; that observation was current as of 2026-07-01T00:00:00Z. Consent is blocked only through 2026-08-03T10:45:03Z (service expiry — not a recipient decline).
